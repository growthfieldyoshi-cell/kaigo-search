...
const slugToName = new Map(SLUG_MAP.map(([name, slug]) => [slug, name]));
const nameToSlug = new Map(SLUG_MAP.map(([name, slug]) => [name, slug]));

export function prefectureFromSlug(slug: string): string | null {
  return slugToName.get(slug) ?? null;
}

export function slugFromPrefecture(name: string): string | null {
  return nameToSlug.get(name) ?? null;
}

export function getAllPrefectureSlugs(): { slug: string; name: string }[] {
  return SLUG_MAP.map(([name, slug]) => ({ slug, name }));
}
