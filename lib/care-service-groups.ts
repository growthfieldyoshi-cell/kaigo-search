...
// 施設詳細ページの冒頭リード文。サービス種別グループごとに、
// この種別の施設をどう比較・検討するときに役立つかを自然な文で示す。
// 個別施設について断定せず、空き状況・費用・医療対応は別途確認すべき旨と整合させる。
export const CARE_SERVICE_GROUP_INTRO: Record<CareServiceGroupKey, string> = {
  residential:
    "特養・老健・グループホーム・有料老人ホームといった入所系の施設を検討する際の比較候補として参考にしてください。空き状況・費用・医療対応・受け入れ条件は施設ごとに大きく異なるため、必ず施設や担当ケアマネジャーへ直接ご確認ください。",
  daycare:
    "日中に通って食事・入浴・機能訓練などを利用する通所系サービスです。送迎範囲や利用可能日、対応できる介護内容は事業所ごとに異なるため、利用前に直接ご確認ください。",
  visit:
    "自宅で生活を続けながら介護・看護・リハビリなどの支援を受けたい方のための訪問系サービスです。訪問対応エリアや時間帯は事業所ごとに異なるため、担当ケアマネジャーや事業所へ直接ご確認ください。",
  consultation:
    "ケアプラン作成や介護サービス利用の相談に対応する居宅介護支援事業所です。要介護認定を受けた方が介護サービスを使い始める際の最初の相談先になります。担当ケアマネジャーの受け入れ状況は事業所へ直接ご確認ください。",
  equipment:
    "介護用ベッド・車いす・歩行器などの福祉用具を貸与または販売する事業所です。取り扱い品目や対応エリア、配送・メンテナンスの体制は事業所ごとに異なるため、利用前にご確認ください。",
  multi_function:
    "通い・訪問・宿泊を組み合わせて利用できる複合型の介護サービスです。利用枠や費用は事業所ごとに異なるため、利用条件を直接ご確認ください。",
  other:
    "介護サービス事業所の基本情報です。サービス内容・利用条件・費用などは事業所ごとに異なるため、利用前に公式情報や事業所へ直接ご確認ください。",
};
